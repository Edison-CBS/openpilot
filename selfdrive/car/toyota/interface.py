...
class CarInterface(CarInterfaceBase):
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), car_fw=[], disable_radar=False):  # pylint: disable=dangerous-default-value
    ...
    if candidate == CAR.PRIUS:
      ...
      set_lat_tune(ret.lateralTuning, LatTunes.TORQUE, lat_params, MAX_LAT_ACCEL=2.8, FRICTION=0.024)
      ...
      if candidate in (CAR.CAMRY_TSS2, CAR.CAMRYH_TSS2):
        set_lat_tune(ret.lateralTuning, LatTunes.TORQUE, lat_params, MAX_LAT_ACCEL=2.4, FRICTION=0.05)
      else:
